...
-- Keep only secure user-specific policies (these already exist)
-- CREATE POLICY "Users can view own themes" ON public.user_themes FOR SELECT USING (user_id = get_current_user_id());
-- CREATE POLICY "Users can insert own themes" ON public.user_themes FOR INSERT WITH CHECK (user_id = get_current_user_id());
-- CREATE POLICY "Users can update own themes" ON public.user_themes FOR UPDATE USING (user_id = get_current_user_id()) WITH CHECK (user_id = get_current_user_id());
-- CREATE POLICY "Users can delete own themes" ON public.user_themes FOR DELETE USING (user_id = get_current_user_id());
...
-- 5. SECURE TEACHERS TABLE - REMOVE BROAD ACCESS
