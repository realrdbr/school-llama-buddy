-- Phase 1: Secure RLS for private messaging and sessions
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE (
      SELECT string_agg('DROP POLICY IF EXISTS ' || quote_ident(polname) || ' ON public.private_conversations;', ' ')
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE (
      SELECT string_agg('DROP POLICY IF EXISTS ' || quote_ident(polname) || ' ON public.private_messages;', ' ')
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE (
      SELECT string_agg('DROP POLICY IF EXISTS ' || quote_ident(polname) || ' ON public.user_contacts;', ' ')
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE (
      SELECT string_agg('DROP POLICY IF EXISTS ' || quote_ident(polname) || ' ON public.user_sessions;', ' ')
...
-- 5) Ensure realtime works for updated_at where needed (optional)
-- Not modifying publication here to avoid side effects.
